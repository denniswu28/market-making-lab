...
def _cast_then_select(
    df: pl.DataFrame, order: List[str], dtypes: Dict[str, pl.DataType]
) -> pl.DataFrame:
    casts = []
    for c in order:
...
def _build_src_paths(conv: ConvertCfg, symbol: str, date_dash: str) -> Dict[str, str]:
    ...
    return {
        ...
        "depth": os.path.join(
            base, "incremental_book_L2", date_dash, f"{symbol}.parquet"
        ),
...
def _latency_out_path(
    lat: LatencyCfg, conv: ConvertCfg, symbol: str, date_ymd: str
) -> str:
    fname = lat.latency_filename_template.replace("{date}", date_ymd)
    out_dir = (
        os.path.join(lat.latency_root, conv.exchange, symbol)
        if lat.include_symbol_dir
        else os.path.join(lat.latency_root, conv.exchange)
    )
    _ensure_dir(out_dir)
...
def _prepare_parquet(
    ...
    if kind == "trades":
        ...
        # normalize side BEFORE casting/selecting
...
def _run_one(job: Job) -> Tuple[str, str, Optional[str], List[str]]:
    ...
    try:
        logs.append(
            f"[START] sym={sym} date={d} quotes={job.conv.use_quotes} "
        ...
        if os.path.exists(feed_out) and job.conv.skip_existing_feed:
            ...
            trades_file = _prepare_parquet(
                src["trades"], tmp_trades, "trades", job.conv.exchange, sym
            )
            depth_file = _prepare_parquet(
                src["depth"], tmp_depth, "depth", job.conv.exchange, sym
            )
            ...
            if job.conv.use_quotes:
                ...
                quotes_file = _prepare_parquet(
                    quotes_path, tmp_quotes, "quotes", job.conv.exchange, sym
                )
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert Tardis parquet to feeds and derive latency — one pipeline."
    )
    ...
    if output_format != "npz":
        raise ValueError(
            "For this pipeline, output_format must be 'npz' (latency step expects npz with key 'data')."
        )
    ...
    conv_cfg_partial = dict(
        ...
        snapshot_mode=snapshot_mode_global,
        sod_ymd=sod_ymd,
        ...
        skip_existing_feed=skip_existing_feed,
    )
    ...
    latency_filename_template: str = cfg.get(
    ...
    lat_cfg_partial = dict(
        ...
        latency_filename_template=latency_filename_template,
    ...
    print(
        f"[pipeline] num_proc={num_proc} buffer_size={buffer_size} ss_buffer_size={ss_buffer_size} base_latency={base_latency}"
    )
    print(
        f"[pipeline] quotes={use_quotes} bucket_every={bucket_every} mul_entry={mul_entry} mul_resp={mul_resp}"
    )
    ...
    with ctx.Pool(processes=num_proc) as pool:
        ...
        for sym, d, err, logs in tqdm(
            it, total=total, desc="Convert → Latency", unit="combo"
        ):
            for line in logs:
                tqdm.write(line)
            if err is None:
